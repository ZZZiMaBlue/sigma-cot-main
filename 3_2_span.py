...
# 设置命令行参数
parser = argparse.ArgumentParser(description='Span Level')
...
parser.add_argument('--input_task', type=str, default='sentence', help='input task: sentence level')
parser.add_argument('--output_task', type=str, default='span', help='output task: span level')
...
# 设置文件路径
data_path = f"/home/zhaoying/pycharm/one_shot/result/3_1_Sentence-Level/{args.dataset}/{args.input_task}_{args.model}.json"
output_path = f"/home/zhaoying/pycharm/one_shot/result/3_2_Span-Level/{args.dataset}"
...
def build_prompt(locate, question, logic, number, dataset):
...
def process_response(response, data, i):
    if response:
        answers_match = re.search(r'(?:##?\s*Answers:|Answers:)\s*(.*?)\s*(?:##?\s*Explanation:|Explanation:|$)',
...
def process_data(data):
    for i in range(len(data)):
        ...
        if ("answer_gpt" in data[i] and data[i]["answer_gpt"] != [""] and data[i]["answer_gpt"] != ["None"] and data[i][
            "answer_gpt"] != []):
            continue

        # if "answer_gpt" not in data[i]:
        #     data[i]["answer_gpt"] = []

        context = data[i]["locate"]
        question = data[i]["question"]
        number = data[i]["answer_number"]
        logic = data[i]["logic"]

        prompt = build_prompt(context, question, logic, number, args.dataset)
        try:
